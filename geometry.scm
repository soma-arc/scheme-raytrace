...
(define (pdf-value obj origin direction)
  ((vector-ref obj 2) origin direction))

(define (random obj origin)
  ((vector-ref obj 3) origin))
...
(define (make-scene obj-list)
  (vector
   ...
   (lambda (origin direction)
     (let* ((num-obj (length obj-list))
            (weight (/ 1 num-obj)))
       (let loop ((i 0) (sum 0))
         (if (= i num-obj)
             sum
             (loop (+ i 1)
                   (+ sum (* weight (pdf-value (ref obj-list i)))))))))
   (lambda (origin)
     (random (ref obj-list (floor->exact (* (random-real)
                                            (length obj-list))))
             origin))
   obj-list))
...
(define (make-sphere center radius material)
  (letrec ((hit (lambda (r t-min t-max)
    ...
    (vector hit
            ...
            (lambda (origin direction)
              (receive (hit? hit-rec)
                       (hit (make-ray origin direction) 0.001 +max-float+)
                       (if hit?
                           (let* ((cos-theta-max (sqrt (- 1 (/ (* radius radius)
                                                               (v:sq-len (v:diff center origin))))))
                                  (solid-angle (* 2 pi (- 1 cos-theta-max)))))
                           0)))
            (lambda (origin)
              (let* ((direction (v:diff center origin))
                     (dist-sq (v:sq-len direction))
                     (uvw (make-onb-from-w direction)))
                (local uvw (random-to-sphere radius dist-sq))))
...
(define (make-xz-rect x0 x1 z0 z1 k material)
  (letrec ((hit (lambda (ray t-min t-max)
              ...
              (receive (hit? hit-rec)
                       ...
                       (if hit?
                           (let* ((area (* (- x1 x0) (- z1 z0)))
                                  (distance-squared (* (t hit-rec) (t hit-rec) (v:sq-len direction)))
                                  (cosine (/ (abs (v:dot direction (normal hit-rec))) (v:length direction))))
                             (/ distance-squared (* cosine area)))
                           0)))
            (lambda (origin)
              (let ((random-point (v:vec3 (+ x0 (* (random-real) (- x1 x0)))
                                          k
                                          (+ z0 (* (random-real) (- z1 z0))))))
                (v:diff random-point origin)))
